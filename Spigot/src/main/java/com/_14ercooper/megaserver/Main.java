		...
		while (true) {
			try {
				...
				while (true) {
					...
					if (profile.equalsIgnoreCase("new profile")) profile = newProfile();
					if (profile.equalsIgnoreCase("exit server")) return;
					inProfile(profile);
				}
			}
			catch (Exception e) {
	...
	private static void inProfile (String profile) throws IOException, InterruptedException {
		...
		while (true) {
			...
			if (input == 1) {
				...
				String command = "java -jar -Xmx" + RAM + "M -Xms" + RAM + "M -Xss" + eighthRam + "M -Xmn" + quarterRam + "M -XX:+UseParallelGC server.jar nogui";
				...
				// Make backup
				FileIO.zipDirectory(new File("profiles/" + profile), new File("backups"), profile, profile + "_" + String.valueOf(Instant.now().getEpochSecond()));
			}
			// Change version
			if (input == 2) {
			...
			if (input == 3) {
				...
				while (true) {
					...
					if (plugin.equalsIgnoreCase("done")) break;
