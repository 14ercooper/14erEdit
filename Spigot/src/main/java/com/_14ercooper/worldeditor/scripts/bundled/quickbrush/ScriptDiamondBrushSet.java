...
public class ScriptDiamondBrushSet extends Craftscript{
	...
	public boolean perform(LinkedList<String> args, Player player, String label) {
		try {
			...
			return player.performCommand("fx br d " + brushRadius + " 0.5 " + opToRun);
		} catch (Exception e) {
			Main.logError("Could not create brush. Did you pass in the correct parameters?", Operator.currentPlayer);
			return false;
		}
	}
}
