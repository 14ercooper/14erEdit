...
import com._14ercooper.worldeditor.operations.type.ColorText;
	...
	public boolean performNode () {
		try {
			...
			if (type.equalsIgnoreCase("num")) {
				...
				Main.logError("Could not modify variable. Modification \"" + mod.get(0) + "\" is not valid.", Operator.currentPlayer);
				return false;
			}
			...
			// Invalid type
			Main.logError("Could not modify variable. Type \"" + type + "\" is not a valid type.", Operator.currentPlayer);
			return false;
		} catch (Exception e) {
			Main.logError("Could not modify variable. Please check your syntax (did you give enough arguments for the modifier?).", Operator.currentPlayer);
			return false;
		}
	}
