...
public class SimplexNode extends Node {
	...
	public SimplexNode newNode() {
		...
		if (node.scaleFactor == null) {
			Main.logError("Could not create simplex node. Three numbers are required, but not provided.", Operator.currentPlayer);
		}
		return node;
	}
	...
	public boolean performNode () {
		try {
			...
			Main.logError("Simplex in " + arg1.getValue() + " dimensions not found. Please check your simplex syntax.", Operator.currentPlayer);
			return false;
		} catch (Exception e) {
