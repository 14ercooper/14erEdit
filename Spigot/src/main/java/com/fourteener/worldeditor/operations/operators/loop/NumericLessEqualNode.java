...
public class NumericLessEqualNode extends Node {
	...
	public NumericLessEqualNode newNode() {
		...
		node.val = GlobalVars.operationParser.parseNumberNode();
			...
			Main.logError("Error creating numeric less than or equal node. Please check your syntax.", Operator.currentPlayer);
			return null;
		}
		if (node.val == null) {
			Main.logError("Could not create numeric less than or equal node. Two arguments required, but not provided.", Operator.currentPlayer);
		}
		return node;
	}
	...
	public boolean performNode () {
		...
		return (Operator.numericVars.get(name).getValue() <= val.getValue());
			...
			Main.logError("Error performing numeric less than or equal node. Please check your syntax (does the variable exist?).", Operator.currentPlayer);
			return false;
		}
	}
