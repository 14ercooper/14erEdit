...
public class SetOperatorVariableNode extends NumberNode {
    ...
    public SetOperatorVariableNode newNode(ParserState parserState) {
        SetOperatorVariableNode node = new SetOperatorVariableNode();
    ...
    public double getValue(OperatorState state) {
        if (child instanceof NumberNode) {
            ...
            state.setVariable(name.getText(), val);
            return val;
        }
        else {
            boolean val = child.performNode(state, true);
            double newVal;
            if (val) {
                newVal = 1;
            }
            ...
            state.setVariable(name.getText(), newVal);
            return newVal;
        }
    }
