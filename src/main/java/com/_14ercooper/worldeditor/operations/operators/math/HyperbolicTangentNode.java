...
public class HyperbolicTangentNode extends NumberNode {

    NumberNode a;

    // Returns a new node
    @Override
    public HyperbolicTangentNode newNode(ParserState parserState) {
        HyperbolicTangentNode node = new HyperbolicTangentNode();
    ...
    public double getValue(OperatorState state) {
        return Math.tanh(a.getValue(state));
    }
