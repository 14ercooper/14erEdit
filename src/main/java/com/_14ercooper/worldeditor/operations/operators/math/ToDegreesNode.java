...
public class ToDegreesNode extends NumberNode {

    NumberNode a;

    // Returns a new node
    @Override
    public ToDegreesNode newNode(ParserState parserState) {
        ToDegreesNode node = new ToDegreesNode();
    ...
    public double getValue(OperatorState state) {
        return Math.toDegrees(a.getValue(state));
    }
