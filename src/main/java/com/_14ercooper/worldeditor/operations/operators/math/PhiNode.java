...
public class PhiNode extends NumberNode {
    // Returns a new node
    @Override
    public PhiNode newNode(ParserState parserState) {
        return new PhiNode();
    }
    ...
    public double getValue(OperatorState state) {
        ...
        return Math.atan(distY / Math.sqrt((distX * distX) + (distY * distY) + (distZ * distZ)));
    }
