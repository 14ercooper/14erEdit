...
public class SecantNode extends NumberNode {

    NumberNode a;

    // Returns a new node
    @Override
    public SecantNode newNode(ParserState parserState) {
        SecantNode node = new SecantNode();
    ...
    public double getValue(OperatorState state) {
        return 1.0 / Math.cos(a.getValue(state));
    }
