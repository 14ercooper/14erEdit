		...
		try {
			...
			if (type.equalsIgnoreCase("itm")) {
				try {
					...
					Main.logError("Error loading varibale - class not found. Is the file a saved item?", Operator.currentPlayer);
					Main.logDebug(e.getMessage());
				}
			}
			if (type.equalsIgnoreCase("mob")) {
				try {
					...
					Main.logError("Error loading varibale - class not found. Is the file a saved monster?", Operator.currentPlayer);
					Main.logDebug(e.getMessage());
				}
			}
			if (type.equalsIgnoreCase("spwn")) {
				try {
					...
					Main.logError("Error loading varibale - class not found. Is the file a saved spawner?", Operator.currentPlayer);
					Main.logDebug(e.getMessage());
				}
			}
			Main.logError("Could not load variable. Only items, monsters, and spawners can be loaded.", Operator.currentPlayer);
			return false;
		} catch (Exception e) {
